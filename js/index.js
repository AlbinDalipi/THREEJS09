...
// find panorama with uuid

    var currentPano = panoramasCollection.find(pano => {
        return pano.get("uuid") === panoUUID;
    });
    if(!currentPano){
...
var portals = currentPano.get("portals");
 ...
 for(var i =0 ; i<portals.length; i++){
     ...
     cube.position.x = portals[i].posx;
     cube.position.y = portals[i].posy;
     cube.position.z = portals[i].posz;
     cube.scale.x = portals[i].scalex;
     cube.scale.y = portals[i].scaley;
     cube.scale.z = portals[i].scalez;
...
var mesh = new THREE.Mesh(new THREE.SphereGeometry(20, 50, 50), sphereMaterial);
mesh.type = "sphericalPano";
...
// https://i.imgur.com/FWMrgKF.jpg
    ...
    if (intersects.length) {
        if(!mouseDown && intersects[0].object.type !== "sphericalPano"){
...
function loadPanoramas(){
    ...
    for (var i  = 0 ; i < dataJSON.panoramas.length; i ++){
        ...
        var panoramaModel = new PanoramaModel({
            ...
            portals: dataJSON.panoramas[i].portals
        }
            );
...
//show spin
                ...
                var currentPano = panoramasCollection.find(model => {
                    return model.get("uuid") === element.dataset.uuid;
                });
                if(currentPano){
                    currenthighres = currentPano.get("highres");
                }
                ...
                loader.load(currenthighres, (texture) => {
                    //hide spin
                    $('#spinner').hide();
                    loadPortals(currentPano.get("uuid"));
...
function ExportToJson(){
    ...
    for (var i = 0 ; i < panoramasCollection.length; i++){


        panoramasCollection.models[i].exportToJSON();
     ...
     for (var i = 0 ; i < objectsCollection.length; i++){
               ...
               objectsCollection.models[i].exportToJSON();
